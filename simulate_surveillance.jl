...
@edit getindex(missing, 2)
spread_reps = 100
traps_per_cells = 1 ./ 10 .^ (-1:0.1:1)
surveillance_reps = 10

NewType = Union{Tuple{Missing,Missing},Tuple{Float64,Float64}}
time_area = NewType[(-1, -1) for i = 1:spread_reps, j = 1:length(traps_per_cells), k = 1:surveillance_reps];
for i = 1:spread_reps
    ...
    for j = 1:length(traps_per_cells)
        for k = 1:surveillance_reps
            ...
            time_area[i, j, k] =
                area_at_detection(output, trappinggrid)
...
for j = 1:length(traps_per_cells)
    ...
    d[j, :areamin] = d[j, :area]  - d[j, :areasd]
    d[j, :areamax] = d[j, :area]  + d[j, :areasd]
    d[j, :time] = mean(skipmissing(detection_time))
    d[j, :timesd] = std(skipmissing(detection_time))
end
CSV.write("simout\\SimulationSummary.csv",  d)

d = CSV.read("simout\\SimulationSummary.csv")

plot(d.traps_per_cell, (d.area), yerror = d.areasd, xlabel = "traps per 9 km cell", color = :blue,    label = "area invaded at detection (ha)")
